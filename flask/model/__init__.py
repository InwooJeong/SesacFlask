...
# 경로 만들기
# print(os.path.dirname(__file__)+'/'+'lang_predict.ml')

# 1. 모델로드
clf = joblib.load(os.path.dirname(__file__)+'/'+'lang_predict.ml')

# 2. 레이블 로드
label = joblib.load(os.path.dirname(__file__)+'/'+'lang_predict.lb')

# 3. 번역 요청 원본 데이터 -> 학습용 전처리 함수
# 텍스트 => 일련의 과정(전처리) => (1, 26), 2차원
def encode_freqs_data(src: str) -> list:
  '''
    - 텍스트 원문 -> 빈도 계산 및 정답 추출 - 특정 구조로 리턴
    - parameters
      - src : 번역 요청 원문 텍스트
    - returns
      - list : [[0.000001,...]]
  ...
  total_count = len(text) # or sum(counts)
...
if __name__ == '__main__':
  # 단위 테스트 용도
  sample_data = '''
    Joseph Crespo dit Monsieur Jo, né le 1er janvier 1925 à Elne (Pyrénées-Orientales) et mort le 7 mai 2004 à Mably (Loire), est un joueur de rugby à XV et international français de rugby à XIII, évoluant au poste de demi de mêlée, de demi d'ouverture ou de centre.
